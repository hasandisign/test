...
def upload_video_to_s3(data1,data2,video_name,video_name2,ACCESS_KEY_ID,ACCESS_SECRET_KEY,BUCKET_NAME):
   ...
   s3.Bucket(BUCKET_NAME).put_object(Key='jpn02/camera0/'+video_name, Body=data1)
   #print("uploading cam 1 to s3")
   s3 = boto3.resource('s3',aws_access_key_id=ACCESS_KEY_ID,aws_secret_access_key=ACCESS_SECRET_KEY,config=Config(signature_version='s3v4'))
   #s3.Bucket(BUCKET_NAME).put_object(Key='jetson101/camera0/'+args.file1+'.mp4', Body=data1)
   s3.Bucket(BUCKET_NAME).put_object(Key='jpn02/camera1/'+video_name2, Body=data2)
...
# Release everything if job is finished
    ...
    if class_id1==0 or class_id1==1:
        ...
        #draw.text(ra_16,"Right ankle", font=fnt,fill=(0,255,0))
            ...
            #print("Red alert")
                ...
                myMQTTClient.publish(topic = "home/zones",QoS=1,payload='{"cameraID": "camera003", "timestamp": '+ str(timestamp) +', "message": "RED ALERT"}')
            if pol_cam1_yellow.contains(pp1):
                cv2.putText(final_img1,"Yellow Aalert",(50,120),cv2.FONT_HERSHEY_SIMPLEX,0.9,(0,255,0),2)
            #print("Yellow alert")
                myMQTTClient.publish(topic = "home/zones",QoS=1,payload='{"cameraID": "camera003", "timestamp": '+ str(timestamp) +', "message": "YELLOW ALERT"}')
            if pol_cam1_green.contains(pp1):
                cv2.putText(final_img1,"Green Alert",(50,140),cv2.FONT_HERSHEY_SIMPLEX,0.9,(0,255,0),2)
            #print("Ggreeen alert")
                myMQTTClient.publish(topic = "home/zones",QoS=1,payload='{"cameraID": "camera003", "timestamp": '+ str(timestamp) +', "message": "GREEN ALERT"}')
    ...
    if class_id2==0 or class_id2==1:
        ...
        #print("Human index:%d "%( i ))
            ...
            #print("Red alert")
                print("publishing message from jetson")
                myMQTTClient.publish(topic = "home/zones",QoS=1,payload='{"cameraID": "camera004", "timestamp": '+ str(timestamp) +', "message": "RED ALERT"}')
            if pol_cam2_red.contains(pp2):
                cv2.putText(final_img2,"Yellow Alert",(50,120),cv2.FONT_HERSHEY_SIMPLEX,0.9,(0,255,0),2)
            #print("Yellow alert")
                myMQTTClient.publish(topic = "home/zones",QoS=1,payload='{"cameraID": "camera004", "timestamp": '+ str(timestamp) +', "message": "YELLOW ALERT"}')
            if pol_cam2_red.contains(pp2):
                cv2.putText(final_img2,"Green Alert",(50,140),cv2.FONT_HERSHEY_SIMPLEX,0.9,(0,255,0),2)
            #print("Ggreeen alert")
                myMQTTClient.publish(topic = "home/zones",QoS=1,payload='{"cameraID": "camera004", "timestamp": '+ str(timestamp) +', "message": "GREEN ALERT"}')
