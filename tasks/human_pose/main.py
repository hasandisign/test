...
net = jetson.inference.imageNet(argv=["--model=" + os.environ['HOME'] + "/Documents/new_classifi/jetson-inference/python/training/classification/models/ttrain_models/resnet50.onnx", "--labels=" + os.environ['HOME'] + "/Documents/new_classifi/jetson-inference/python/training/classification/data/ttrain_data/labels.txt", "--input-blob=input_0", "--output-blob=output_0", os.environ['HOME'] + "/src/trt_pose/tasks/human_pose/train_scenario2.mp4"])
...
myMQTTClient.configureCredentials(os.environ['HOME'] + "/jpn01_cert/AmazonRootCA1.pem", os.environ['HOME'] + "/jpn01_cert/private.pem.key", os.environ['HOME'] + "/jpn01_cert/certificate.pem.crt")
...
parser.add_argument('--video1', type=str, default=os.environ['HOME'] + '/src/trt_pose/tasks/human_pose/train_scenario2.mp4', help = 'video file name' )
parser.add_argument('--video2', type=str, default=os.environ['HOME'] + '/src/trt_pose/tasks/human_pose/station_video_final.mp4', help = 'video file name' )
parser.add_argument('--image', type=str, default=os.environ['HOME'] + '/src/trt_pose/tasks/human_pose/check.png', help = 'image file name' )
...
out_video1 = cv2.VideoWriter(os.environ['HOME'] + '/src/trt_pose/tasks/human_pose/output_video/%s_%s.mp4'%(args.model, name), fourcc1, cap1.get(cv2.CAP_PROP_FPS), (W, H))
...
out_video2 = cv2.VideoWriter(os.environ['HOME'] + '/src/trt_pose/tasks/human_pose/output_video/%s_%s.mp4'%(args.model, name), fourcc2, cap2.get(cv2.CAP_PROP_FPS), (W, H))
X_compress = 762 / WIDTH * 1.0     #to draw at exact key point 800.0 is the widht of source video
Y_compress = 1012 / HEIGHT * 1.0    #to draw at exact key point 600.0 is the height of source video
...
input = jetson.utils.videoSource('/dev/video0', argv=sys.argv)   #/dev/video0 for camera
output = jetson.utils.videoOutput(opt.output_URI, argv=sys.argv+is_headless)
font = jetson.utils.cudaFont()
while True:
    ret_val1, dst1 = cap1.read()
        ...
        #print("Human index:%d "%( i ))
            ...
            for k in range(0, len(new_original_roi2)):
                ...
                if poly2.contains(pp2) and k==1:
                    cv2.putText(final_img2,"Yellow Aalert",(50,120),cv2.FONT_HERSHEY_SIMPLEX,0.9,(0,255,0),2)
                #print("Yellow alert")
    ...
    #out_video1.write(final_img1)       # THIS ONLY TO SAVE THE final inferred VIDEO
#client.loop_stop()
#client.disconnect()
